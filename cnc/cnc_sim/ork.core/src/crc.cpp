...
const uint16_t CCRC::crc_table[256] = 
{
...
void CCRC::Add( uint16_t & _v, uint8_t _d )
{
...
void CCRC::Add( uint16_t & _v, int32_t _d )
{
...
void CCRC::Add( uint16_t & _v, int16_t _d )
{
...
uint32_t CCRC::HashStringCaseInsensitive( const char * _string )
{
    if ( !_string )
    {
		return( 0 );
    ...
    crc_final = uint32_t((crc_16s[0] << 16) | crc_16s[1]);
...
uint32_t CCRC::HashStringCaseSensitive( const char * _string )
{
    if ( !_string )
    {
		return( 0 );
...
uint32_t CCRC::HashMemory( const void * _address, int32_t _length )
{
	if ( !_address )
    {
		return( 0 );
