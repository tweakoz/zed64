...
void toolmesh::Prune()
{
	std::set<std::string> SubsToPrune;
...
{	if(1) return;//
	...
	static int icnt=0;
	std::string fname = FormatString("tmeshout%d.txt", icnt );
	icnt++;
	...
	{	const submesh& src_grp = *it->second;
		...
		std::set<const AnnoMap*>	annosets;
		for( int ip=0; ip<inump; ip++ )
		{	const poly& ply = src_grp.RefPoly(ip);
			const AnnoMap* amap = ply.GetAnnoMap();
			if(amap) annosets.insert(amap);
		}
		for( std::set<const AnnoMap*>::const_iterator it2=annosets.begin(); it2!=annosets.end(); it2++ )
		{	const AnnoMap* amapp = (*it2);
			const std::map<std::string,std::string>& amap	= amapp->mAnnotations;
			for( std::map<std::string,std::string>::const_iterator it3=amap.begin(); it3!=amap.end(); it3++ )
...
{	static const char* defret( "" );
	std::map<std::string,std::string>::const_iterator it = mAnnotations.find( std::string(annokey) );
...
{	OrkAssert( std::vector<int>::size_type(i) < mMergedPolys.size() );
...
const std::vector<poly>& submesh::RefPolys() const
{	return mMergedPolys;
}
...
/*const std::vector<submesh *> &toolmesh::RefPolyGroupByPolyIndex() const
{
...
void submesh::FindNSidedPolys( std::vector<int>& output, int inumsides ) const
...
void submesh::GetEdges( const poly& ply, std::vector<edge>& Edges ) const
{	int icnt = 0;
	int icntf = 0;
...
void submesh::GetAdjacentPolys( int ply, std::set<int>& output ) const
{	std::vector<edge> edges;
	GetEdges(RefPoly(ply), edges);
	for (std::vector<edge>::const_iterator edgeIter = edges.begin(); edgeIter != edges.end(); edgeIter++)
	{	std::set<int> connectedPolys;
		GetConnectedPolys(*edgeIter,connectedPolys);
		for( std::set<int>::const_iterator it2=connectedPolys.begin(); it2!=connectedPolys.end(); it2++ )
		{	int  ic = *it2;
			if( ic != ply )
...
void submesh::GetConnectedPolys( const edge & ed, std::set<int>& output ) const
...
{	std::map<std::string,std::string>::const_iterator it = mAnnotations.find(annoname);
	if( it!=mAnnotations.end() )
	{	return it->second;
	}
	return gnomatch;
...
{	const AnnoMap* rval = 0;
	...
	std::map<U64,const AnnoMap*>::const_iterator it = mAnnoMap.find(uhash);
	if( it != mAnnoMap.end() )
	{	rval = it->second;
	}
	return rval;
