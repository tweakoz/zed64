...
public:
	...
	void deallocate(pointer p)
	{
		OrkAssert(owned(p));
		m_free.push_back(p);
		auto it = std::find( m_used.begin(), m_used.end(), p );
	...
	void clear()
	{
		m_used.clear();
		m_free.clear();
		for( typename std::vector<value_type>::iterator it = m_data.begin(); it != m_data.end(); it++)
			m_free.push_back(&*it);
	}

	const std::vector<pointer>& used() const { return m_used; }
...
protected:
	...
	std::vector<pointer>    m_free;
	std::vector<pointer>    m_used;
