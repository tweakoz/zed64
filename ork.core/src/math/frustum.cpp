...
void Frustum::Set( const mtx44_type& IVPMatrix )
{
    ...
    vec4_type camrayHALF = (camrayN+camrayF)*0.5f;
	...
	vec3_type  inNormal = mZNormal*-1.0f;
	mNearPlane.CalcFromNormalAndOrigin( mZNormal, camrayN );
    mFarPlane.CalcFromNormalAndOrigin( inNormal, camrayF );

	double t = KEPSILON;
    mTopPlane.CalcPlaneFromTriangle( mFarCorners[1], mFarCorners[0], mNearCorners[0], KEPSILON );
    mBottomPlane.CalcPlaneFromTriangle( mNearCorners[3], mFarCorners[3], mFarCorners[2],KEPSILON );
    mLeftPlane.CalcPlaneFromTriangle( mNearCorners[0], mFarCorners[0], mFarCorners[3] ,KEPSILON );
    mRightPlane.CalcPlaneFromTriangle( mNearCorners[2], mFarCorners[2], mFarCorners[1],KEPSILON );
	//CalcCorners()l
    ...
    printf( "Dn %f Df %f Dt %f Db %f Dl %f Dr %f\n", Dn, Df, Dt, Db, Dl, Dr );
#endif
}
